...
import random
from zoo import get_input_shape, preprocess_input_overall
import paths
# import tifffile
...
INPUT_PATH = paths.input_path + 'input/'
...
def get_indexes():
    return ['Cardiomegaly', 'Effusion', 'Emphysema', 'Hernia', 'Mass', 'Nodule', 'Infiltration',
            'Atelectasis', 'Pneumothorax', 'Pleural_Thickening', 'Fibrosis', 'Edema', 'Consolidation', 'Pneumonia',
            'No_Finding']
...
def get_train_label_matrix():
    tbl = pd.read_csv(INPUT_PATH + "data.csv")
    labels = tbl['classes'].apply(lambda x: x.split('|'))
    ...
    files = []
    for id in tbl['img'].values:
        files.append("./input/images/" + id + '.png')
