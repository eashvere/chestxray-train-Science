...
def fire_module(x, fire_id, squeeze=16, expand=64, dim_ordering='th'):
    ...
    right = Convolution2D(
        expand, 3, 3, border_mode='same', name=s_id + exp3x3)(x)
    right = Activation('relu', name=s_id + relu + exp3x3)(right)

    x = merge([left, right], mode='concat',
              concat_axis=c_axis, name=s_id + 'concat')
    return x
...
def get_squeezenet_top(dim_ordering='th'):
    ...
    x = Convolution2D(64, 3, 3, subsample=(
        2, 2), border_mode='valid', name='conv1')(input_img)
